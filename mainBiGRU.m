...
for jj = 1:21
    ...
    layers = addLayers(layers, [
        sequenceInputLayer(f_, "Name", "input")                    % Input layer
        gruLayer(6, 'OutputMode', 'last', "Name", "gru1")          % GRU layer
        concatenationLayer(1, 2, "Name", "cat")                    % Concatenation
        fullyConnectedLayer(outdim, "Name", "fc1")                 % Fylly connnected layer
        QRegressionLayer('out', 0.5)]);                            % Regression layer

    layers = addLayers(layers, [
        FlipLayer("flip")                                          % Flip layer
        gruLayer(6, 'OutputMode', 'last', "Name", "gru2" )] );     % Backward GRU
    ...
    t_sim1(jj, :) = predict(net, p_train); 
    t_sim2(jj, :) = predict(net, p_test ); 
